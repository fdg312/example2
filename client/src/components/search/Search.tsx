...
export const Search = () => {
    ...
    useEffect(() => {
        ...
        if (selectValue) {
            router.push('/categories/' + getCategory(selectValue)?.slug + '/' + selectValue)
        }
    }, [selectValue])
    ...
    return (
        <div>
            <CategorySelect categories={categories} disabled={disabled} styles={styles.select} value={selectValue} onChange={e => setSelectValue(e.target.value)} />
            <div className="relative inline">
                <SearchInput setValue={setSearchValue} value={searchValue} type="text" text="Поиск..." />
                <div className="absolute top-[38px] w-full left-0 ps-3 bg-white">
                {
                    typeof searchRes === 'object' && searchRes.map((add: IAdd) => (
                        <Link href={'/?query=' + add.title}  key={add.title} className=" border-[#555555] border-b-2 block">{add.title}</Link>
                    ))
                }
                </div>
            </div>
